...
Error::Error(DNSServiceErrorType error) noexcept
{
    if (error != kDNSServiceErr_NoError)
    {
        ...
        errorMessage
                << "DNSServiceError: ("
                << Error::DNSServiceErrorDescription(error)
                << ")";
...
{
    if (mErrorMsg.empty()) { return "no error"; }
    return mErrorMsg;
}
...
const char *Error::DNSServiceErrorDescription(DNSServiceErrorType error) noexcept {
    switch (error)
    {
        ...
        case kDNSServiceErr_Incompatible:               return "kDNSServiceErr_Incompatible (client library incompatible with daemon)";
        ...
        case kDNSServiceErr_ServiceNotRunning:          return "kDNSServiceErr_ServiceNotRunning (background daemon not running)";
        case kDNSServiceErr_NATPortMappingUnsupported:  return "kDNSServiceErr_NATPortMappingUnsupported (NAT doesn't support PCP, NAT-PMP or UPnP)";
        case kDNSServiceErr_NATPortMappingDisabled:     return "kDNSServiceErr_NATPortMappingDisabled (NAT supports PCP, NAT-PMP or UPnP, but it's disabled by the administrator)";
        case kDNSServiceErr_NoRouter:                   return "kDNSServiceErr_NoRouter (No router currently configured (probably no network connectivity)";
...
#ifndef _WIN32
        case kDNSServiceErr_DefunctConnection:          return "kDNSServiceErr_DefunctConnection (connection to daemon returned a SO_ISDEFUNCT error result)";
#endif
